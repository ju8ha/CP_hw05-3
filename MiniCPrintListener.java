...
public class MiniCPrintListener extends MiniCBaseListener {
	...
	public void exitFun_decl(MiniCParser.Fun_declContext ctx) {
		...
		newTexts.put(ctx, s1 + " " + s2 + L_paren + s4 + R_paren + "\r\n" + s6);
	}
	...
	public void exitWhile_stmt(MiniCParser.While_stmtContext ctx) {
		...
		newTexts.put(ctx, s1 + " " + L_paren + s2 + R_paren + "\n" + s3);
	}
	...
	public void exitLocal_decl(MiniCParser.Local_declContext ctx) {	// 지역변수
		...
		if (ctx.getChildCount() == 3) {
			...
			s2 = ctx.getChild(1).getText();
			semi = ctx.getChild(2).getText();
			newTexts.put(ctx, s1 + " " + s2 + semi);	// 모두 더한 값을 newTexts에 저장
			...
			semi = ctx.getChild(4).getText();
			newTexts.put(ctx, s1 + " " + s2 + " " + s3 + " " + s4 + semi);	// 모두 더한 값을 newTexts에 저장
	...
	public void exitIf_stmt(MiniCParser.If_stmtContext ctx) {
		...
		if (ctx.children.size() == 5) {
			...
			newTexts.put(ctx, s1 + " " + L_paren + exp + R_paren + "\n" + st);	// 모두 더한 값을 newTexts에 저장
		}
		else{
			...
			newTexts.put(ctx, s1 + " " + "\n" + L_paren + exp + "\n" + R_paren + st + s2 + st2);	// 모두 더한 값을 newTexts에 저장
		}
	}
	...
	public void exitExpr(MiniCParser.ExprContext ctx) {
		...
		if (ctx.getChildCount() == 1) {
			...
			if (isBinaryOperation(ctx)) {
				...
				// expr EQ expr || expr NE expr || expr LE expr || expr GE expr || expr AND expr || expr OR expr
				// expr '<' expr || expr '>' expr
				s1 = newTexts.get(ctx.expr(0));
				s2 = newTexts.get(ctx.expr(1));
				op = ctx.getChild(1).getText();
				newTexts.put(ctx, s1 + " " + op + " " + s2);	// 모두 더한 값을 newTexts에 저장
			} else {
	...
	private String getNumOfDot(int a) {	// nesting 수를 입력받아서 ....을 반환하는 메소드
		String s = "";
		for (int i = 0; i < a; i++)
			s += "....";
		return s;
